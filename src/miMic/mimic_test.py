...
def load_from_folder(samba_output, folder, tag):
    ...
    mutual_index = list(set(names).intersection(tag.index))
    tag = tag.loc[mutual_index]
    names = mutual_index
    tag = tag.loc[names]
...
def calculate_all_imgs_tag_corr(samba_output, name, folder, tag, start_i, eval="corr", sis='fdr_bh',
                                correct_first=False,
                                mode="test", threshold_p=0.05, THRESHOLD_edge=0.5, shuffle=False, colorful=None,
                                save_folder='plots', strain_flag=False):
    """
    ...
    if mode == "plot":
        if shuffle == False:
            ...
            SIZE = 15
            if strain_flag:
                column_labels = ['Kingdom', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species', 'Strain']
            else:
            ...
            if strain_flag:
                ...
                line_styles = ['-', '--', '-.', ':', '-', '--', '-.']
            if strain_flag:
                line_widths = [1, 1.5, 2, 1, 1.5, 2, 1, 1.5]
            else:
            ...
            if strain_flag:
                df_hists['Strain'] = img_arrays[:, 8, :].flatten()
            ...
            for df_corr_name in enumerate(df_corss.index):
                ...
                for col_index, value in enumerate(cell_row):
                    ...
                    if df_corr_name_o in value:
                        ...
                        if strain_flag:
                            if (len_name_dfcorr == 8) or (
                                    len_name_dfcorr < 8 and not (bact_names[len_name_dfcorr + 1, col_index].isalpha())):
                                cell_col = col_index
                                break
                            else:
                                continue
                        elif (len_name_dfcorr == 7) or (
...
def apply_nested_anova(samba_output, folder, tag, mode="test", eval="man", threshold_p=0.05, strain_flag=False):
    """
    ...
    if strain_flag:
        taxonomy_level = ['Anaerobic', 'Kingdom', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species', 'Strain']
        p_vals_df = pd.DataFrame(index=[1, 2, 3, 4, 5, 6, 7, 8], columns=["nested-p"])
    else:
...
def plot_rp_sp_anova_p(df, mixed, save, strain_flag=False):
    """
    ...
    mpl.rc('font', family='Times New Roman')
    if strain_flag:
        taxonomy_level = ['Kingdom', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species', 'Strain', mixed]
    else:
...
def calculate_rsp(df, save):
    ...
    RP = df["RP"][len(df.index)]
    SP = df["SP"][len(df.index)]
    if RP == 0 and SP == 0:
...
def apply_mimic(name, folder, tag, eval="man", sis="fdr_bh", correct_first=True, mode="test", save=True, tax=None,
                colorful=True, threshold_p=0.05, THRESHOLD_edge=0.5, rawData=None, taxnomy_group="sub PCA",
                preprocess='True', processed=None, apply_samba=True, samba_output=None,save_folder='plots',strain_flag=False):
    """
    ...
    if strain_flag:
        ...
        spec.loader.exec_module(mip) # Load the module
        taxonomy_level = 8

    else:
        # Standard absolute import
        mip = importlib.import_module("MIPMLP")
        taxonomy_level = 7
    ...
    with warnings.catch_warnings():
        ...
        if mode == "preprocess":
            if preprocess:
                ...
                rawData = rawData.rename(columns={rawData.columns[0]: 'ID'})
                ...
                if strain_flag:
                    ...
                    taxonomy_level=7
                    strain_flag = False
                processed = mip.preprocess(rawData, taxnomy_group=taxnomy_group,
                                    taxonomy_level=taxonomy_level)  # log mean normalization
                if processed.shape[1] < 10:
                    processed = mip.preprocess(rawData, taxnomy_group=taxnomy_group, taxonomy_level=7)
                    strain_flag= False

                print("Preprocessing is done.")
                return processed, strain_flag
            else:
                return

        save_folder = f"{save_folder}/{name}"
        ...
        if mode == 'test':
            ...
            p = apply_nested_anova(samba_output, folder, tag, mode=mode, eval=eval, threshold_p=threshold_p, strain_flag=strain_flag)
            if p > threshold_p:
                ...
                for t1 in [1, 2, 3]:
                    print(f"\nTaxonomy is {t1}")
                    df_corrs123 = calculate_all_imgs_tag_corr(samba_output, name, folder, tag, t1, eval=eval,
                                                              sis=sis, correct_first=correct_first, mode=mode,
                                                              threshold_p=threshold_p,
                                                              shuffle=False, save_folder=save_folder)
            ...
            df_corrs_leaves = calculate_all_imgs_tag_corr(samba_output, name, folder, tag, 0, eval=eval,
            ...
            if t1 != 'noAnova':
                ...
                if save:
                    df_corrs.to_csv(f"{save_folder}/df_corrs.csv")
                    df_corrs_leaves_difference.to_csv(f"{save_folder}/u_test_without_mimic.csv")
                    df_corrs123.to_csv(f"{save_folder}/just_mimic.csv")
                    mimic_and_utest.to_csv(f"{save_folder}/miMic&Utest.csv")
                ...
                if save:
                    df_corrs_leaves.to_csv(f"{save_folder}/u_test_without_mimic.csv")
                    df_corrs_leaves.to_csv(f"{save_folder}/df_corrs.csv")
                    df_corrs= df_corrs_leaves

            return t1, samba_output,df_corrs
